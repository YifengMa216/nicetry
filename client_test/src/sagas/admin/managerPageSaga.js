...
import { actionTypes as ArticleTypes } from "../../reducers/admin/managerPage";
import { actionTypes as NewArticleTypes } from "../../reducers/admin/managerNewPage";

export function* getArticleList(pageNum) {
    yield put({ type: IndexActionTypes.FETCH_START });
    try {
        return yield call(get, `/getPages?pageNum=${pageNum}`);
    } catch (err) {
        yield put({
            type: IndexActionTypes.SET_MESSAGE,
            msgContent: "getPages网络请求错误",
            msgType: 0,
        });
    } finally {
    ...
    while (true) {
        ...
        if (res) {
            if (res.code === 0) {
                ...
                yield put({
                    type: ArticleTypes.ADMIN_RESPONSE_GET_ARTICLE_LIST,
                    data: res.data,
                });
...
export function* deleteArticle(id) {
    yield put({ type: IndexActionTypes.FETCH_START });
    try {
        return yield call(get, `/admin/page/delPage?id=${id}`);
    } catch (err) {
        yield put({
            type: IndexActionTypes.SET_MESSAGE,
            msgContent: "网络请求错误",
...
export function* deletePageFlow() {
    while (true) {
        ...
        if (res) {
            if (res.code === 0) {
                ...
                yield put({
                    type: ArticleTypes.ADMIN_GET_ARTICLE_LIST,
                    pageNum,
                });
            } else if (res.message === "身份信息已过期，请重新登录") {
                yield put({
